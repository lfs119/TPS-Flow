...
parser.add_argument('--mddir', type=str, default='/data/cb/scratch/share/tps_flow/4AA_sims')
...
from tps_flow.wrapper import Newtps_flowWrapper
...
def do(model, name, seqres):
    ...
    if os.path.exists(f'{args.out_dir}/{name}_metadata.pkl'):
        ...
        with temp_seed(137):
            # get_featurized_traj_rMD
            feats, ref = tps_flow.analysis.get_featurized_traj_rMD(f'{os.path.dirname(args.mddir)}/renum_topol_noH.pdb', f'{args.mddir}/{xtc_name}.xtc',sidechains=True)
            # feats, ref = tps_flow.analysis.get_featurized_traj(f'{args.mddir}/{name}/{name}', sidechains=True)
            tica, _ = tps_flow.analysis.get_tica(ref, lag=20)
            kmeans, ref_kmeans = tps_flow.analysis.get_kmeans(tica.transform(ref), k=20)
            try:
                msm, pcca, cmsm = tps_flow.analysis.get_msm(ref_kmeans, lag=20, nstates=10)
    ...
    arr = np.lib.format.open_memmap(f'{args.data_dir}/{xtc_name}.npy', 'r')
    ...
    for i in tqdm.tqdm(range(args.num_batches), desc='num batch'):
        ...
        for j in range(args.batch_size):
            ...
            tmp_path = os.path.join(args.out_dir, f'{idx}')
            if not os.path.exists(tmp_path):
                os.mkdir(tmp_path)
            path = os.path.join(tmp_path, f'{name}_{idx}.pdb')
            atom14_to_pdb(atom14s[j].cpu().numpy(), batch['seqres'][0].cpu().numpy(), path)

            traj = mdtraj.load(path)
            traj.superpose(traj)
            traj.save(os.path.join(tmp_path, f'{name}_{idx}.xtc'))
            traj[0].save(os.path.join(tmp_path, f'{name}_{idx}.pdb'))
            metadata.append({
                'name': name,
...
@torch.no_grad()
def main():
    model = Newtps_flowWrapper.load_from_checkpoint(args.sim_ckpt)
    model.eval().to('cuda')
