...
from tps_flow.model.vfn_ipa_wrapper import create_vfn_score_model

from .custom_utils import TPA, DyT, FreBlock, FreBlock2, ProteinSpatioTemporalAttention, ProteinSpatioTemporalAttention2, ProteinSpatioTemporalAttention3, ProteinSpatioTemporalAttention5, ProteinSpatioTemporalAttention6, ProteinSpatioTemporalAttention7, ProteinSpatioTemporalAttention8, make_temporal_window
...
from .ipa import EnhancedIPA, EnhancedIPA2, EnhancedIPA3, InvariantPointAttention
...
class Latent_Flow_Model(nn.Module):
    def __init__(self, args, latent_dim):
        ...
        if hasattr(self.args, 'energy'):
            if self.args.energy:
                self.energy_to_emb = nn.Linear(1, args.embed_dim)
        ...
        if self.args.toker_ecoder:
            self.layers = nn.ModuleList([
                ProteinSpatioTemporalAttention7(args.embed_dim, num_heads=8, time_heads=8) for _ in range(args.num_layers)
            ])

  

        if self.args.temporal_window:
            self.temporal_window_dense = nn.Linear(3 * args.embed_dim, args.embed_dim)
            self.norm = nn.LayerNorm(args.embed_dim * 3, eps=1e-12)
            self.norm1 = nn.LayerNorm(args.embed_dim, eps=1e-12)
            self.dense1_act = get_activation('gelu')
        ...
        # add recontruction layer     0322
        if self.args.recon:
          self.emb_to_recon = FinalLayer(args.embed_dim, latent_dim)

        if self.args.separation_head:
            self.emb_to_sep = FinalLayer(args.embed_dim, 6)
    ...
    def initialize_weights(self):
        ...
        if self.args.prepend_ipa:
            for block in self.ipa_layers:
                if hasattr(block, "ipa.linear_out"):
    ...
    def forward(self, x, t, mask,
        ...
        if self.args.energy:
            x = x + self.energy_to_emb(energy).unsqueeze(-2)

        if self.args.fft or self.args.fft2:
            x = self.fre_block(x)
        
        if self.args.toker_ecoder:
            for layer_idx, layer in enumerate(self.layers):
         
                 x, _ = layer(x, mask, t, None, None)
        else:
            for layer_idx, layer in enumerate(self.layers):
        ...
        B, T, L, C = x.shape
        if self.args.temporal_window:
            temporal_w_out = make_temporal_window(x, T)
            temporal_w_out_enocder = self.temporal_window_dense(self.norm(temporal_w_out))
            x = self.norm1(self.dense1_act(temporal_w_out_enocder))
        ...
        if self.args.recon:
            recon_pre = self.emb_to_recon(x, t)
        else:
            recon_pre = None
        if self.args.separation_head:
            refine_tra = self.emb_to_sep(x, t)
        else:
            refine_tra = None
        ...
        return latent, recon_pre, refine_tra
    ...
    def forward_inference(self, x, t, mask,
        ...
        if not self.args.design or self.args.dynamic_mpnn or self.args.mpnn:
            return self.forward(x, t, mask, start_frames, end_frames, x_cond, x_cond_mask, aatype, energy)
        else:
            x_discrete = x[:, :, :, -20:]
            ...
            latent = self.forward(x, t, mask, start_frames, end_frames, x_cond, x_cond_mask, aatype, energy)
...
class IPALayer(nn.Module):
    ...
    def _init_submodules(self, add_bias_kv=False, dropout=0.0, ipa_args=None):
        ...
        # self.ipa = EnhancedInvariantPointAttention(**ipa_args)
        # self.ipa = EnhancedIPA3(**ipa_args)
...
class Latenttps_flowLayer(nn.Module):
    """Transformer layer block."""
    ...
    def _init_submodules(self, add_bias_kv=False, dropout=0.0, ipa_args=None):
        ...
        if self.hyena:
            ...
            self.mha_t = TPA(self.embed_dim, self.mha_heads)
        ...
        self.mha_l = TPA(self.embed_dim, self.mha_heads)
