...
from .model.latent_model import Latent_Flow_Model
...
class Wrapper(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_item = self.general_step(batch, stage='val')
    ...
    #     scheduler = {
    #     'scheduler': CosineAnnealingLR(optimizer, T_max=self.args.epochs, eta_min=self.args.lr/10),  
    #     'interval': 'epoch',  
    #     'frequency': 1,       
...
class NewMDGenWrapper(Wrapper):
    def __init__(self, args):
        ...
        self.model = Latent_Flow_Model(args, latent_dim)
    ...
    def prep_batch(self, batch):
        ...
        offsets[..., :4] *= torch.where(offsets[:, :, :, 0:1] < 0, -1, 1) #（B,T,L,7）


        
        #0402  test only
        # offsets[:, 1:, :, 4:7] = (offsets[:, 1:, :, 4:7] - 0.0086) / 0.5230



        # 0331
        if self.args.replace_frames:
            offsets[:, 1:, :, 4:7] = (offsets[:, 1:, :, 4:7] - 1.462) / 60.915
        if self.args.offsets_zero:
            offsets[:, 1:, :, 4:7] = offsets[:, 1:, :, 4:7] * 0
        ...
        if self.args.tps_condition or self.args.inpainting or self.args.dynamic_mpnn:
            ...
            #0402 test only
            # offsets_r[:, :-1, :, 4:7] = (offsets_r[:, :-1, :, 4:7] - 0.0086) / 0.5230



            # 0331
            if self.args.replace_frames:
               offsets_r[:, :-1, :, 4:7] = (offsets_r[:, :-1, :, 4:7] - 1.462) / 60.915
              
            if self.args.offsets_zero:
                offsets_r[:, 1:, :, 4:7] = offsets_r[:, 1:, :, 4:7] * 0


            offsets = torch.cat([offsets, offsets_r], -1)
            frame_loss_mask = torch.cat([frame_loss_mask, frame_loss_mask], -1)   # B, L, 14
        ...
        return {
            ...
            'model_kwargs': {
                ...
                'x_cond': torch.where(cond_mask.unsqueeze(-1).bool(), latents, 0.0),
                'x_cond_mask': cond_mask,
                'energy': batch['energy'].unsqueeze(-1) if batch['energy'] is not None else None,
            }
        }
    ...
    def inference(self, batch):
        ...
        # #0402  test only
        # offsets[:, 1:, :, 4:7] = (offsets[:, 1:, :, 4:7] * 0.5230)  +  0.0086

        # 0331
        if self.args.replace_frames:
            offsets[:, 1:, :, 4:7] = offsets[:, 1:, :, 4:7] * 60.915 + 1.462 
        if self.args.offsets_zero:
            offsets[:, 1:, :, 4:7] = offsets[:, 1:, :, 4:7] * 0
...
class TPS_Flow(NewMDGenWrapper):
    def __init__(self, args):
        super().__init__(args)
