...
class Transport:
    ...
    def training_losses(
        ...
        model_output, recon, refine_tra = model(xt, t, **model_kwargs)
        B, *_, C = xt.shape
        ...
        terms = {}
        terms['t'] = t
        # model_output[:, :, :, 4:7] += refine_tra[:, :, :, :4]
        # model_output[:, :, :, 11:14] += refine_tra[:, :, :, 4:]
        ...
        if not (self.args.dynamic_mpnn or self.args.mpnn):
            if self.model_type == ModelType.VELOCITY:
                ...
                if recon is not None:
                    if epoch > self.args.thr_epoch:
                        terms['recon_loss'] = 0.2 * mean_flat(((recon - x1) ** 2), mask)
                        terms['loss'] = 0.8 * mean_flat(((model_output - ut) ** 2), mask) + terms['recon_loss']
                    else:
                        terms['loss'] = mean_flat(((recon - x1) ** 2), mask)
                elif refine_tra is not None:
                    key1_tra_res= ut[:, :, :, 4:7] - model_output[:, :, :, 4:7]
                    key2_tra_res= ut[:, :, :, 11:14] - model_output[:, :, :, 11:14]
                    key_tra_res = torch.cat([key1_tra_res, key2_tra_res], dim=-1)
                    key_mask = torch.cat([mask[:, :, :, 4:7], mask[:, :, :, 11:14]], dim=-1)
                    terms['loss'] = mean_flat(((model_output - ut) ** 2), mask) + mean_flat(((key_tra_res - refine_tra) ** 2), key_mask)
                else:
                    terms['loss'] = mean_flat(((model_output - ut) ** 2), mask)
    ...
    def get_drift(
        ...
        def noise_ode(x, t, model, **model_kwargs):
            ...
            model_output, _, _= model(x, t, **model_kwargs)
            score = model_output / -sigma_t
            return (-drift_mean + drift_var * score), _ , _
        ...
        def body_fn(x, t, model, **model_kwargs):
            model_output, _, refine_tra = drift_fn(x, t, model, **model_kwargs)
            assert model_output.shape == x.shape, "Output shape from ODE solver must match input shape"
            if refine_tra is not None:
                model_output[:, :, :, 4:7] += refine_tra[:, :, :, :3]
                model_output[:, :, :, 11:14] += refine_tra[:, :, :, 3:6]
            return model_output
